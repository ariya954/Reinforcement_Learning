...
"""
from environments.dog_mdp import DogMDPEnv
import numpy as np
import algorithms
import utils
...
if __name__ == '__main__':
    ...
    num_runs = 100
    env = DogMDPEnv()
    random_policy = algorithms.create_random_policy(env)

    # Table 4.1
    first_visit_V = []
    every_visit_V = []
    for i in range(num_runs):
        V = algorithms.mc_policy_evaluation(env, random_policy, discount, num_episodes, first_visit=True)
        first_visit_V.append(V)
        V = algorithms.mc_policy_evaluation(env, random_policy, discount, num_episodes, first_visit=False)
    ...
    for i in range(num_runs):
        ...
        V = algorithms.incremental_mc_policy_evaluation(env, random_policy, discount, num_episodes, first_visit=True)
    ...
    incr_first_visit_Q = []
    for i in range(num_runs):
        Q = algorithms.incremental_mc_policy_evaluation_qpi(env, random_policy, discount, num_episodes, first_visit=True)
    ...
    V = algorithms.incremental_mc_policy_evaluation(env, random_policy, discount, num_episodes, first_visit=False)
