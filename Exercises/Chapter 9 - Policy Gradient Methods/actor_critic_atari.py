...
class ActorCriticAgent:
    ...
    def choose_action(self, observation):
        ...
        s_t = torch.from_numpy(observation_np[None, ...]).to(device=self.device, dtype=torch.float32)
...
def run_train_loop(env, agent, num_train_steps):
    ...
    while t < num_train_steps:
        s_t = gym_env_processor.AtariResetWrapper.reset(env)
        r_t = 0
        done = False
        ...
        while True:
            ...
            s_t, r_t, done, info = gym_env_processor.AtariStepWrapper.step(env, a_t)
            ...
            if done:
                ...
                unused_a_t = agent.act(s_t, r_t, done)  # noqa: F841
                break
...
def run_evaluation_loop(env, agent, num_eval_steps):
    ...
    for _ in range(num_eval_steps):
        a_t = agent.act(s_t)
        s_t, r_t, done, _ = gym_env_processor.AtariStepWrapper.step(env, a_t)
